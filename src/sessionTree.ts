...
export function groupSessionsIntoRoots(
  ...
  function placeRow(row: SessionTreeRow): { key: string; label: string } {
  ...
  const byRoot = new Map<string, { key: string; label: string; rows: SessionTreeRow[] }>();
 ...
 * the row carries.
 */
function buildDirectories(rows: SessionTreeRow[], home: string | null): SessionDirectory[] {
