...
/** The SSH round trip used to gather every available TCP listener view. */
export const PORT_LISTENER_SCAN_COMMAND = [
  'echo "<<<PS_SS_TLN>>>"; ss -tln 2>/dev/null;',
  'echo "<<<PS_SS_TLNP>>>"; ss -tlnp 2>/dev/null;',
  'echo "<<<PS_NETSTAT_TLNP>>>"; netstat -tlnp 2>/dev/null;',
  'echo "<<<PS_NETSTAT_TLN>>>"; netstat -tln 2>/dev/null;',
  'true',
].join(' ');

/** Outcome of one listener scan; a failed scan is never evidence of absence. */
...
/**
 * Combine the sentinel-delimited listener output. `ss -tln` is the source of
 * truth for the complete port list: `ss -tlnp` can omit sockets it cannot
 * attribute. The `-p` variants enrich that list by port number.
 */
export function mergePortScanSections(stdout: string): RemotePort[] {
  ...
  return ports.map((port) => {
    const extra = attribution.get(port.port);
    if (!extra) return port;
    return {
      ...port,
      process: port.process ?? extra.process,
      pid: port.pid ?? extra.pid,
    };
  });
}

/**
 * Interpret one SSH command result. Missing sentinels and transport failures
 * remain distinct from a successful listing that found no usable rows.
 */
export function parsePortScanResult(
  stdout: string,
  result: { exitCode: number | null; stderr?: string; timedOut?: boolean },
): PortScanResult {
  if (result.timedOut) {
    return { ok: false, ports: [], error: 'port scan timed out' };
  }
  if (result.exitCode !== 0) {
    return {
      ok: false,
      ports: [],
      error: result.stderr?.trim() || `port scan exited ${String(result.exitCode)}`,
    };
  }
  if (Object.keys(splitSections(stdout)).length === 0) {
    return { ok: false, ports: [], error: 'port scan produced no output' };
  }
  return { ok: true, ports: mergePortScanSections(stdout), error: null };
}

/** Build a safe `/proc/<pid>/cwd` query for PIDs parsed from remote output. */
export function procCwdCommand(pids: readonly number[]): string {
  const safe = pids.filter((pid) => Number.isInteger(pid) && pid > 0);
