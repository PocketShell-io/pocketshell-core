/**
 * Directory-derived session naming — folder-name validation and path joining.
 ...
 * sessionNameParts.ts — the renderer's folder workspace labels its tabs by
 * stripping a folder's derived base name off its sessions, so it needs the
 * derivation itself, not merely the sanitiser, and two implementations of a
 * rule this exact would drift.
 */

import { trimTrailingSlash } from './sessionNameParts';
 ...
 * quoting in projectCommands.ts is that.
 */
