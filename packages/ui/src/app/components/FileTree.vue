...
/**
 * The tree's model — rendered rows (cap + search), keyboard navigation, the
 * create flow, and the editable path bar — lives in useFileTreeModel.ts, with
 * its decision records. This component keeps the gestures, the menus, and the
 * breadcrumb strip.
 */
const {
  query,
...
// The create flow and its two doors live in useFileTreeModel.ts, with the
// reasoning for the inline naming row.
...
// useFileTreeModel.ts — the roving-tabindex scheme and its reasoning.
...
// The row cap + search state is the model's (`useFileTreeModel.ts`); the cap
// is a RENDER cap, not a fetch limit — `fileListView.ts` carries the reasoning.
